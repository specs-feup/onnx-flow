...
class JsonToOnnxConverter {
    static convert(jsonFilePath, outputOnnxFilePath) {
        ...
        return new Promise((resolve, reject) => {
            ...
            protobuf.load(protoPath, (err, root) => {
                ...
                function loadAndConvertJson(filePath) {
                    try {
                        ...
                        if (jsonObject.graph) {
                            ...
                            if (jsonObject.graph.node) {
                                jsonObject.graph.node.forEach((node) => {
                                    if (node.attribute) {
                                        node.attribute.forEach((attr) => {
                                            if (attr.ints) {
                                                attr.ints = attr.ints.map((val) => parseInt(val, 10));
                                            }
                                            if (attr.floats) {
                                                attr.floats = attr.floats.map((val) => parseFloat(val));
                                            }
                                        });
                                    }
                                });
...
//# sourceMappingURL=json2onnx.js.map