...
function parseShape(shape) {
    return shape.dim.map((dim) => parseInt(dim.dimValue, 10));
}
let definedVars = [];
// Add input nodes to the graph
function addInputNodes(data, graph) {
...
function addOutputNodes(data, graph) {
...
function addNodes(data, graph, mapNodeAndOutput, mapNodeAndInputs) {
    let index = 0;
    const nodesToAdd = new Set(data.graph.node.map((_, i) => i));
    const addedNodes = new Set();
    while (nodesToAdd.size > 0) {
        for (const nodeIndex of nodesToAdd) {
            const node = data.graph.node[nodeIndex];
            const allInputsDefined = node.input.every((input) => definedVars.includes(input));
            if (allInputsDefined) {
...
function addEdges(graph, mapNodeAndOutput, mapNodeAndInputs) {
...
function findDims(outputNodes, graph) {
...
//# sourceMappingURL=initGraph.js.map