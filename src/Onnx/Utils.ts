...
export function maybeRemoveOrphanConstant(g: OnnxGraph.Class, tn?: TensorNode.Class) {
    ...
    const consumers = toArrayLike<OperationNode.Class>(
        tn.getOutgoers?.targets?.filterIs?.(OperationNode),
    );
    if (consumers.length > 0) return;

    // remove upstream Constant op if it's now orphan
    const srcOps = toArrayLike<OperationNode.Class>(
        tn.getIncomers?.sources?.filterIs?.(OperationNode),
    );
    for (const src of srcOps) {
        if (src.type !== "Constant") continue;
        const outs = toArrayLike<TensorNode.Class>(
            src.getOutgoers?.targets?.filterIs?.(TensorNode),
        );
        const stillUsed = outs.some(
            () => toArrayLike(src.getOutgoers?.targets?.filterIs?.(OperationNode)).length > 0,
        );
...
export function isNumeric(dtype: DataType): boolean {
    ...
    return !(dtype === (DataType as any).STRING || dtype === (DataType as any).BOOL);
}
...
export function findTensorByOnnxName(
    ...
    t = pool.find((n) =>
        (n as any).extraAttrs?.some?.((a: any) => a.name === "name" && a.s === name),
    );
    return t;
}
...
export function findConstantProducerAsTensor(
    ...
    for (const c of allOps) {
        ...
        const outs: string[] =
            (c as any).outputs ?? (c as any).output ?? (c as any).getOutputNames?.() ?? [];
        if (outs[0] === onnxName) {
...
export function readConstIntegerVectorFromTensorNode(tn?: TensorNode.Class): number[] | undefined {
    ...
    if (raw) {
        ...
        for (let i = 0; i < n; i++) {
            const off = i * elemBytes;
            if (isI64)
                out.push(Number(dv.getBigInt64(off, true))); // little-endian int64
...
export function readScalarFromTensorNode(tn?: TensorNode.Class): number | undefined {
    ...
    if (raw) {
        let u8: Uint8Array;
        if (raw instanceof Uint8Array) u8 = raw;
        else if ((globalThis as any).Buffer?.isBuffer(raw))
            u8 = new Uint8Array(raw.buffer, raw.byteOffset, raw.byteLength);
        else if (Array.isArray(raw)) u8 = Uint8Array.from(raw);
        else return undefined;
        if (u8.byteLength === 8) {
            const dv = new DataView(u8.buffer, u8.byteOffset, u8.byteLength);
            try {
                return Number(dv.getFloat64(0, true));
            } catch {
                /* noop */
            }
            try {
...
export function editShapeDim(
    ...
    const sc = g
        ...
        .init(new OperationNode.Builder("ScatterElements", [baseShape, idx, size1D], { axis: 0 }))
        .as(OperationNode);
    const out = g
...
export function makeTensorConst(
    ...
    const builder =
        tensorKind === "constant"
            ? new TensorNode.Builder(dataType, proto.dims!, tensorKind, proto)
            : new TensorNode.Builder(dataType, proto.dims!, tensorKind, undefined, proto);
...
export function swap<A>(a: A[], i: number, j: number) {
    const x = a[i];
    a[i] = a[j];
    a[j] = x;
    return a;
