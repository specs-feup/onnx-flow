...
export function inferNodeShape(node: OperationNode.Class, graph: OnnxGraph.Class): void {
    ...
    switch (node.type) {
        ...
        case "Transpose": {
            ...
            const perm = getAttr(node, "perm", inputShape.map((_, i) => i).reverse()) as number[];
            ...
            if (target.length > 0) {
                if (inputShape.length > 0) {
                    ...
                    resolved.forEach((d, i) => {
                        if (d === 0) resolved[i] = inNums[i] ?? 1;
                    });
                    resolved.forEach((d, i) => {
                        if (d === -1) {
                            if (inferIndex !== -1)
                                throw new Error("Reshape: multiple -1 in target");
                            inferIndex = i;
                        } else {
                            knownProd *= d || 1;
                        }
        ...
        case "Unsqueeze": {
            ...
            if (axesNode !== undefined && axesNode.is(ConstantNode)) {
                raw =
                    decodeIntegerVectorFromTensorProto(axesNode.as(ConstantNode).constantValue) ??
                    [];
            }
        ...
        case "Gather": {
            ...
            outShape = [...dataShape.slice(0, axis), ...indicesShape, ...dataShape.slice(axis + 1)];
        ...
        case "Slice": {
            ...
            if (startsNode !== undefined && startsNode.is(ConstantNode)) {
                starts =
                    decodeIntegerVectorFromTensorProto(startsNode.as(ConstantNode).constantValue) ??
                    [];
            }
            if (endsNode !== undefined && endsNode.is(ConstantNode)) {
                ends =
                    decodeIntegerVectorFromTensorProto(endsNode.as(ConstantNode).constantValue) ??
                    [];
            }
            if (axesNode !== undefined && axesNode.is(ConstantNode)) {
                axes =
            ...
            if (stepsNode !== undefined && stepsNode.is(ConstantNode)) {
                steps =
                    decodeIntegerVectorFromTensorProto(stepsNode.as(ConstantNode).constantValue) ??
                    [];
            }
        ...
        case "AveragePool": {
            ...
            const n = x[0],
                ...
                h = x[2],
                wdim = x[3];
            const kernel = (getAttr(node, "kernel_shape", [1, 1]) as number[] | undefined) ?? [];
    ...
    //console.log(`[InferShapes] Op: ${node.id} (${node.type}) -> inferred outShape: [${outShape}]`);
}
