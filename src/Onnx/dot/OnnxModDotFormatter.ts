...
export default class OnnxModDotFormatter<
...
> extends DefaultDotFormatter<G> {
    ...
    static defaultGetNodeAttrs(node: BaseNode.Class): Record<string, string> {
        ...
        node.switch(
            Node.Case(TensorNode, (n) => {
                ...
                result.address = n.address.toString();
            }),
    ...
    nodeToDot(node: BaseNode.Class): DotNode {
        return Dot.node(this.idPrefix + node.id, OnnxModDotFormatter.defaultGetNodeAttrs(node));
    }
    ...
    edgeToDot(edge: BaseEdge.Class): DotEdge {
        ...
        const dot = Dot.edge(
            source,
            target,
            {...OnnxModDotFormatter.defaultGetEdgeAttrs(edge), ...attrs}
        );
        return dot;
    }
    ...
    loopBodyToDot(node: OperationNode.Class): {DotSubgraph : DotSubgraph, idPrefix : string} | null {
        ...
        if (body && body instanceof OnnxGraph.Class) {
            ...
            const subFormatter = new OnnxModDotFormatter(
                OnnxModDotFormatter.defaultGetNodeAttrs,
                OnnxModDotFormatter.defaultGetEdgeAttrs,
                DefaultDotFormatter.defaultGetContainer,
                OnnxModDotFormatter.defaultGetGraphAttrs,
                idPrefix
            );
    ...
    ifBodyToDot(node: OperationNode.Class): {thenSubgraph?: DotSubgraph, elseSubgraph?: DotSubgraph, idPrefix: string} {
        ...
        if (thenBody && thenBody instanceof OnnxGraph.Class) {
            const thenFormatter = new OnnxModDotFormatter(
                OnnxModDotFormatter.defaultGetNodeAttrs,
                OnnxModDotFormatter.defaultGetEdgeAttrs,
                DefaultDotFormatter.defaultGetContainer,
                OnnxModDotFormatter.defaultGetGraphAttrs,
        ...
        if (elseBody && elseBody instanceof OnnxGraph.Class) {
            const elseFormatter = new OnnxModDotFormatter(
                OnnxModDotFormatter.defaultGetNodeAttrs,
                OnnxModDotFormatter.defaultGetEdgeAttrs,
                DefaultDotFormatter.defaultGetContainer,
                OnnxModDotFormatter.defaultGetGraphAttrs,
        ...
        for (const node of graph.nodes) {
            ...
            if (isIntermediateTensor) {
                ...
                for (const inEdge of incomers) {
                    ...
                    const edgeAttrs = OnnxModDotFormatter.defaultGetEdgeAttrs(inEdge);
