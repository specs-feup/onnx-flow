...
> extends DefaultDotFormatter<G> {
    ...
    loopToDot(node: OperationNode.Class): DotStatement[] {
        ...
        if (body !== undefined) {
            ...
            const carry = body.nodes.filter((node, _a, _b) => node.id.startsWith("carry")).first();
            const carryOut = body.nodes
                .filter((node, _a, _b) => node.id.startsWith("carry_out"))
                .first();
    ...
    toIgnore(node: DotNode): boolean {
        ...
        if (
            id.startsWith("loop") &&
            (id.includes("_cond_in") || id.includes("_cond_out") || label === "Identity")
        ) {
            return true;
        }
