...
> extends DefaultDotFormatter<G> {
  ...
  ifToDot(node: OperationNode.Class): DotStatement[] {
    ...
    if (thenBranch !== undefined) {
      ...
      const thenEdge = Dot.edge(this.idPrefix + node.id, thenIdPrefix + firstThenNode.id)
    ...
    if (elseBranch !== undefined) {
      ...
      const elseEdge = Dot.edge(this.idPrefix + node.id, elseIdPrefix + firstElseNode.id) // Linked to first node in subgraph
