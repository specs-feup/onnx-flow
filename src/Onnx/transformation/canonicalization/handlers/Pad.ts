...
function ensurePadSlabConst(
    ...
    const exp = g
        .addNode(uniq(g, `pad_expand_${tag}`))
        .init(new OperationNode.Builder("Expand", [kT, newShape], {}))
        .as(OperationNode);
    ...
    const slab = g
        .addNode(uniq(g, `pad_slab_${tag}`))
        .init(new TensorNode.Builder(dtype, slabShape, "intermediate"))
        .as(TensorNode);
...
function ensureEdgeSlab(
    ...
    const gdim = g
        .addNode(uniq(g, `edge_gather_${tag}`))
        .init(new OperationNode.Builder("Gather", [shp, axisIdx], { axis: 0 }))
        .as(OperationNode);
    const dim1D = g
        .addNode(uniq(g, `edge_dim1D_${tag}`))
    ...
    if (tag.endsWith("L")) {
        ...
        // Right slab
        const start1 = g
            .addNode(uniq(g, `edge_start1_${tag}`))
            .init(new OperationNode.Builder("Sub", [dim1D, one1], {}))
            .as(OperationNode);
        const start1T = g
            .addNode(uniq(g, `edge_start1T_${tag}`))
            .init(new TensorNode.Builder(DataType.INT64, [1], "intermediate"))
            .as(TensorNode);
    ...
    const oneSlice = g
        .addNode(uniq(g, `edge_oneSlice_${tag}`))
...
function ensureReflectSlab(
    ...
    // dim1D: [dim]
    const gdim = g
        .addNode(uniq(g, `refl_gather_${tag}`))
        .init(new OperationNode.Builder("Gather", [shp, axisIdx], { axis: 0 }))
        .as(OperationNode);
    const dim1D = g
        .addNode(uniq(g, `refl_dim1D_${tag}`))
        .init(new TensorNode.Builder(DataType.INT64, [1], "intermediate"))
        .as(TensorNode);
    addEdge(g, gdim, dim1D, DataType.INT64, [1]);
    ...
    const dimSq = g
        .addNode(uniq(g, `refl_dim_sq_${tag}`))
        .init(new OperationNode.Builder("Squeeze", [dim1D, dimAxes], {}))
        .as(OperationNode);
    const dimSc = g
        .addNode(uniq(g, `refl_dim_scalar_${tag}`))
    ...
    const dimMinus1Op = g
        .addNode(uniq(g, `refl_dim_minus1_${tag}`))
        .init(new OperationNode.Builder("Sub", [dimSc, oneSc], {}))
        .as(OperationNode);
    const dimMinus1 = g
        .addNode(uniq(g, `refl_dim_minus1T_${tag}`))
        .init(new TensorNode.Builder(DataType.INT64, [], "intermediate"))
        .as(TensorNode);
    ...
    const sizeClampOp = g
        .addNode(uniq(g, `refl_size_clamp_${tag}`))
        .init(new OperationNode.Builder("Min", [sizeSc, dimMinus1], {}))
        .as(OperationNode);
    const sizeClamped = g
        .addNode(uniq(g, `refl_size_clamped_${tag}`))
        .init(new TensorNode.Builder(DataType.INT64, [], "intermediate"))
        .as(TensorNode);
    ...
    const rangeOp = g
        .addNode(uniq(g, `refl_range_${tag}`))
    ...
    const gatherOp = g
        .addNode(uniq(g, `refl_gather_data_${tag}`))
...
export default function padHandler(g: OnnxGraph.Class, op: OperationNode.Class): boolean {
    ...
    for (let ax = 0; ax < rank; ax++) {
        ...
        // end = dim - negE (as [1])
        const gdim = g
            .addNode(uniq(g, `pad_crop_gdim_${op.id}_${ax}`))
            .init(new OperationNode.Builder("Gather", [shp, axVec], { axis: 0 }))
            .as(OperationNode);
        const dim1D = g
            .addNode(uniq(g, `pad_crop_dim1D_${op.id}_${ax}`))
            .init(new TensorNode.Builder(DataType.INT64, [1], "intermediate"))
            .as(TensorNode);
        addEdge(g, gdim, dim1D, DataType.INT64, [1]);
        const negE1 = makeI64ShapeConst(g, `pad_crop_nege_${op.id}_${ax}`, [negE]);
        const endOp = g
            .addNode(uniq(g, `pad_crop_endop_${op.id}_${ax}`))
            .init(new OperationNode.Builder("Sub", [dim1D, negE1], {}))
            .as(OperationNode);
        const end1 = g
            .addNode(uniq(g, `pad_crop_end_${op.id}_${ax}`))
            .init(new TensorNode.Builder(DataType.INT64, [1], "intermediate"))
            .as(TensorNode);
        ...
        const sliceOp = g
            .addNode(uniq(g, `pad_crop_slice_${op.id}_${ax}`))
            .init(new OperationNode.Builder("Slice", [cur, start1, end1, axVec], {}))
            .as(OperationNode);
        const mid = g
            .addNode(uniq(g, `pad_crop_mid_${op.id}_${ax}`))
            .init(new TensorNode.Builder(dtype, [...currentShape], "intermediate"))
            .as(TensorNode);
    ...
    for (let ax = 0; ax < rank; ax++) {
        ...
        if (mode === "constant") {
            if (pBeg > 0)
                left = ensurePadSlabConst(g, cur, ax, pBeg, dtype, padValue, `${op.id}_${ax}_L`);
            if (pEnd > 0)
                right = ensurePadSlabConst(g, cur, ax, pEnd, dtype, padValue, `${op.id}_${ax}_R`);
        } else if (mode === "edge") {
        ...
        const cc = g
            .addNode(uniq(g, `pad_concat_${op.id}_${ax}`))
        ...
        if (ax === rank - 1) {
            ...
            const mid = g
                .addNode(uniq(g, `pad_mid_${op.id}_${ax}`))
                .init(new TensorNode.Builder(dtype, [...currentShape], "intermediate"))
                .as(TensorNode);
