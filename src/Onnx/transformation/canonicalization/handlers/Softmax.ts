...
export default function softmaxHandler(g: OnnxGraph.Class, op: OperationNode.Class): boolean {
    ...
    const xShape = inShape.length ? [...inShape] : [];
    const redShape =
        rank === 0
            ? []
            : (() => {
    ...
    // ---- sh = Sub(X, m)
    const sub = g
        .addNode(uniq(g, `sm_sub_${op.id}`))
        .init(new OperationNode.Builder("Sub", [X, M], {}))
        .as(OperationNode);
    const SH = g
        .addNode(uniq(g, `sm_shT_${op.id}`))
    ...
    // ---- ex = Exp(sh)
    const exp = g
        .addNode(uniq(g, `sm_exp_${op.id}`))
        .init(new OperationNode.Builder("Exp", [SH], {}))
        .as(OperationNode);
    const EX = g
        .addNode(uniq(g, `sm_exT_${op.id}`))
        .init(new TensorNode.Builder(X.literalType, xShape, "intermediate"))
        .as(TensorNode);
    ...
    // ---- den = ReduceSum(ex, axes=[axis], keepdims=1)
    const rsum = g
        .addNode(uniq(g, `sm_rsum_${op.id}`))
        .init(
            new OperationNode.Builder(
                "ReduceSum",
                [EX, makeI64ShapeConst(g, `sm_axes_${op.id}`, [axis])],
                { keepdims: 1 },
            ),
        )
        .as(OperationNode);
    const DEN = g
        .addNode(uniq(g, `sm_denT_${op.id}`))
        .init(new TensorNode.Builder(X.literalType, redShape, "intermediate"))
        .as(TensorNode);
