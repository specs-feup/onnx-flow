...
export default function quantizeLinearHandler(
    ...
    if (isPerAxis) {
        ...
        // Unsqueeze S
        const uSop = g
            .addNode(uniq(g, `QL_unsqS_${op.id}`))
            .init(new OperationNode.Builder("Unsqueeze", [S, axes]))
            .as(OperationNode);
        const Sranked = g
            .addNode(uniq(g, `QL_Srank_${op.id}`))
        ...
        // Unsqueeze Z
        const uZop = g
            .addNode(uniq(g, `QL_unsqZ_${op.id}`))
            .init(new OperationNode.Builder("Unsqueeze", [Zf, axes]))
            .as(OperationNode);
        const Zranked = g
            .addNode(uniq(g, `QL_Zrank_${op.id}`))
            .init(new TensorNode.Builder(floatT, [], "intermediate"))
            .as(TensorNode);
        ...
        // Expand S
        const expSop = g
            .addNode(uniq(g, `QL_ExpandS_${op.id}`))
            .init(new OperationNode.Builder("Expand", [Sranked, shapeX], {}))
            .as(OperationNode);
        Sx = g
            .addNode(uniq(g, `QL_Sx_${op.id}`))
        ...
        // Expand Z
        const expZop = g
            .addNode(uniq(g, `QL_ExpandZ_${op.id}`))
            .init(new OperationNode.Builder("Expand", [Zranked, shapeX], {}))
            .as(OperationNode);
        Zx = g
            .addNode(uniq(g, `QL_Zx_${op.id}`))
            .init(new TensorNode.Builder(floatT, X.shape, "intermediate"))
            .as(TensorNode);
    ...
    // 3. Div = X / Scale
    const divOp = g
        .addNode(uniq(g, `QL_Div_${op.id}`))
        .init(new OperationNode.Builder("Div", [X, Sx], {}))
        .as(OperationNode);
    const divOut = g
        .addNode(uniq(g, `QL_DivOut_${op.id}`))
        .init(new TensorNode.Builder(floatT, X.shape, "intermediate"))
        .as(TensorNode);
    ...
    // 4. Round
    const roundOp = g
        .addNode(uniq(g, `QL_Round_${op.id}`))
        .init(new OperationNode.Builder("Round", [divOut], {}))
        .as(OperationNode);
    const roundOut = g
        .addNode(uniq(g, `QL_RoundOut_${op.id}`))
        .init(new TensorNode.Builder(floatT, X.shape, "intermediate"))
        .as(TensorNode);
    ...
    // 5. Add Zero Point
    const addOp = g
        .addNode(uniq(g, `QL_Add_${op.id}`))
        .init(new OperationNode.Builder("Add", [roundOut, Zx], {}))
        .as(OperationNode);
    const addOut = g
        .addNode(uniq(g, `QL_AddOut_${op.id}`))
        .init(new TensorNode.Builder(floatT, X.shape, "intermediate"))
        .as(TensorNode);
    ...
    const clipOp = g
        .addNode(uniq(g, `QL_Clip_${op.id}`))
        .init(new OperationNode.Builder("Clip", [addOut, minT, maxT], {}))
        .as(OperationNode);
    const clipOut = g
        .addNode(uniq(g, `QL_ClipOut_${op.id}`))
        .init(new TensorNode.Builder(floatT, X.shape, "intermediate"))
        .as(TensorNode);
    ...
    const finalCastOp = g
        .addNode(uniq(g, `QL_FinalCast_${op.id}`))
