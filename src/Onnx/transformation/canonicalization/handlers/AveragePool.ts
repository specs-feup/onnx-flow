...
export default function averagePoolHandler(g: OnnxGraph.Class, op: OperationNode.Class): boolean {
    ...
    // ---------- SPECIAL CASE: tiled global-like AvgPool ----------
    ...
    if (useAutoPad) {
        ...
        const pads = rawPads.length > 0 ? rawPads.map(Number) : is2D ? [0, 0, 0, 0] : [0, 0];
    ...
    if (countIncludePad === 1) {
        ...
        const div = g
            .addNode(uniq(g, `AvgPool_DivC_${op.id}`))
        ...
        const expand = g
            ...
            .as(OperationNode);
        const OnesX = g
            .addNode(uniq(g, `AvgPool_OnesX_${op.id}`))
        ...
        const Denom = g
            .addNode(uniq(g, `AvgPool_Denom_${op.id}`))
            .init(new TensorNode.Builder(dtype, Y.shape, "intermediate"))
            .as(TensorNode);
        ...
        // Y = SumOut / Denom
        const div = g
            .addNode(uniq(g, `AvgPool_DivM_${op.id}`))
