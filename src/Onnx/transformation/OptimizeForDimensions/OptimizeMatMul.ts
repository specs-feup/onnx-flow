...
export default function optimizeMatMul(node: OperationNode.Class, graph: OnnxGraph.Class): void {
    ...
    let jNode, kNode, columns1Node, rows1Node, 
    displacementInMemoryNode, multiplication0Node, multiplication1Node, multiplication5Node, 
    index0Node, index1Node, indexResNode, addition0Node, addition1Node, addition3Node;
    ...
    switch (pattern) {
        ...
        case '101':
            ...
            graph.addEdge(indexNode, index0Node).init(new OnnxInnerEdge.Builder(order++)).as(OnnxInnerEdge);
            graph.addEdge(displacementInMemoryNode, index0Node).init(new OnnxInnerEdge.Builder(order++)).as(OnnxInnerEdge);

            graph.addEdge(indexNode, index1Node).init(new OnnxInnerEdge.Builder(order++)).as(OnnxInnerEdge);
            graph.addEdge(displacementInMemoryNode, index1Node).init(new OnnxInnerEdge.Builder(order++)).as(OnnxInnerEdge);
    ...
    if (index0Node && index1Node && indexResNode) {
        ...
        if (kNode && jNode) {
            ...
            if (pattern === '010' && columns1Node) graph.addEdge(columns1Node, equality0Node).init(new OnnxInnerEdge.Builder(order++)).as(OnnxInnerEdge);
           
            else if (rows1Node) graph.addEdge(rows1Node, equality0Node).init(new OnnxInnerEdge.Builder(order++)).as(OnnxInnerEdge);
