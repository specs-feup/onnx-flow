...
export function decomposeMatMul(
    ...
    if (input1.shape.length > 2 || input2.shape.length > 2) {
        throw new Error(
            "MatMul decomposition for tensors with more than 2 dimensions is not supported.",
        );
    }
    ...
    for (let row = 0; row < numRows; row++) {
        ...
        for (let col = 0; col < numCols; col++) {
            ...
            const mulOutputEdgeBuilder = new OnnxEdge.Builder(literalType, [input1.shape[1]]);
            g.addEdge(mulNode, mulOutputNode).init(mulOutputEdgeBuilder);

            // Create ReduceSum node
            const reduceSumBuilder = new OperationNode.Builder("ReduceSum", [mulOutputNode], {
                keepdims: 0,
            });
            ...
            const resultElementBuilder = new TensorNode.Builder(literalType, [], "intermediate");
            const reduceSumOutput = g
                .addNode(`${node.id}_reducesum_out${row}${col}`, node.parent)
                .init(resultElementBuilder)
                .as(TensorNode);

            const reduceSumOutputEdgeBuilder = new OnnxEdge.Builder(literalType, []);
        ...
        const unsqueezeOutputEdgeBuilder = new OnnxEdge.Builder(literalType, [1]);
        ...
        if (unsqueezes.length === 1) {
            ...
            const unsqueezeOutputBuilder = new TensorNode.Builder(literalType, [1], "intermediate");
            ...
            const concatOutEdgeBuilder = new OnnxEdge.Builder(literalType, [numCols]);
