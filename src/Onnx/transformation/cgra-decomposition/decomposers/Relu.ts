...
export default function decomposeRelu(
    ...
    if (input.shape.length > 2) {
        throw new Error(
            "Relu decomposition for tensors with more than 2 dimensions is not supported.",
        );
    }
    ...
    for (let i = 0; i < inputs.length; i++) {
        ...
        const greaterBuilder = new OperationNode.Builder("Greater", [inputs[i], reluZero]);
        const greaterNode = g
        ...
        const greaterOutEdgeBuilder = new OnnxEdge.Builder(DataType.BOOL, inputs[i].shape.slice());
        g.addEdge(greaterNode, greaterOut).init(greaterOutEdgeBuilder);

        // Create Where node (serving as "mux" node)
        const whereBuilder = new OperationNode.Builder("Where", [greaterOut, inputs[i], reluZero]);
        const whereNode = g
            .addNode(`${node.id}_where${i}`, node.parent)
            .init(whereBuilder)
            .as(OperationNode);

        const whereOutEdgeBuilder = new OnnxEdge.Builder(output.literalType, output.shape.slice(1));
