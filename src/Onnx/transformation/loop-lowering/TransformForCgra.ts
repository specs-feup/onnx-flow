...
function splitInput(
    ...
    if (input.type !== "input") {
        ...
        const inputBuilder = new TensorNode.Builder(literalType, newShape, "intermediate");
        ...
        if (numDivs > 1) {
            ...
            const split = g.addNode(`${input.id}_split`).init(splitBuilder).as(OperationNode);
            ...
            const squeeze = g.addNode(`${input.id}_squeeze`).init(squeezeBuilder).as(OperationNode);
        ...
        if (numDivs > 1) {
            for (let i = 0; i < numDivs; i++) {
                const inputBuilder = new TensorNode.Builder(literalType, newShape, input.type);
                const newInput = g
                    .addNode(input.id + i.toString())
                    .init(inputBuilder)
                    .as(TensorNode);
...
function mergeOutputs(
    ...
    for (let i = 0; i < outputs.length; i++) {
        ...
        const unsqOutBuilder = new TensorNode.Builder(literalType, [1, 1], "intermediate");
        const unsqOut = g.addNode(`${outputs[i].id}_unsq_out`).init(unsqOutBuilder).as(TensorNode);
    ...
    const concat = g.addNode(`${originalOutput.id}_concat`).init(concatBuilder).as(OperationNode);

    // Add Concat output node
    const concatOutBuilder = new TensorNode.Builder(literalType, [outputs.length], "intermediate");
    const concatOut = g
