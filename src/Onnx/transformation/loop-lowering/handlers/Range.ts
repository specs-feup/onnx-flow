...
export default function handleRange(
    ...
    const iterCastN = g
        .addNode(uniq(g, `range_iter_cast_${op.id}`))
        .init(new OperationNode.Builder("Cast", [ctx.iter], { to: yTy }))
        .as(OperationNode);
    const iterCast = g
        .addNode(uniq(g, `range_iter_cast_out_${op.id}`))
        .init(new TensorNode.Builder(yTy, [], "intermediate"))
        .as(TensorNode);
    g.addEdge(iterCastN, iterCast)
        .init(new OnnxEdge.Builder(iterCast.literalType, iterCast.shape))
        .as(OnnxEdge);

    const mulN = g
        .addNode(uniq(g, `range_mul_${op.id}`))
        .init(new OperationNode.Builder("Mul", [iterCast, delta]))
        .as(OperationNode);
    const mul = g
        .addNode(uniq(g, `range_mul_out_${op.id}`))
        .init(new TensorNode.Builder(yTy, [], "intermediate"))
        .as(TensorNode);
    g.addEdge(mulN, mul).init(new OnnxEdge.Builder(mul.literalType, mul.shape)).as(OnnxEdge);

    const addN = g
        .addNode(uniq(g, `range_add_${op.id}`))
        .init(new OperationNode.Builder("Add", [start, mul]))
        .as(OperationNode);
    const y = g
        .addNode(uniq(g, `range_y_${op.id}`))
        .init(new TensorNode.Builder(yTy, [], "intermediate"))
        .as(TensorNode);
