...
function sliceBatchThenReshape2D(
    ...
    if (batch.length > 0) {
        ...
        for (let ax = 0; ax < batch.length; ax++) {
            ...
            g.addEdge(gather, out)
                .init(new OnnxEdge.Builder(out.literalType, out.shape))
                .as(OnnxEdge);
            cur = out;
        }
    }
    ...
    const resh = g
        .addNode(uniq(g, `reshape2D_${tag}`))
        .init(new OperationNode.Builder("Reshape", [cur, shape2]))
        .as(OperationNode);
    const out2d = g
        .addNode(uniq(g, `reshape2D_out_${tag}`))
        .init(new TensorNode.Builder(cur.literalType, shape2.shape, "intermediate"))
        .as(TensorNode);
...
function shapeVec2(
    ...
    const cat = g
        .addNode(uniq(g, `concat_${name}`))
        .init(new OperationNode.Builder("Concat", [d0v, d1v], { axis: 0 }))
        .as(OperationNode);
    const out = g
        .addNode(uniq(g, `concat_out_${name}`))
...
export default function handleMatMul(
    ...
    // Common constants
    const M_c = isKnownDim(M)
        ? scalarI64(g, `M_${op.id}`, Number(M))
        : gatherDim(g, `M_${op.id}`, lhsTensor, -2);
    const K_c = isKnownDim(K)
        ? scalarI64(g, `K_${op.id}`, Number(K))
        : gatherDim(g, `K_${op.id}`, lhsTensor, -1);
    const N_c = isKnownDim(N)
        ? scalarI64(g, `N_${op.id}`, Number(N))
  ...
  */
    ...
    if (!ctx.coalesce) {
        ...
        // B2D: [K,N] -> col j -> [K,1]
        const [, colGathered] = gatherFrom(g, B2D, `gather_${B2D.id}_${op.id}`, jU, 1);
        ...
        const iMulN = g
            ...
            .as(TensorNode);
        g.addEdge(iMulN_node, iMulN)
            .init(new OnnxEdge.Builder(iMulN.literalType, iMulN.shape))
            .as(OnnxEdge);
        ...
        const flat_no_batch = g
            ...
            .init(new TensorNode.Builder(DataType.INT64, iIdx.shape, "intermediate"))
            .as(TensorNode);
        g.addEdge(flat_no_batch_node, flat_no_batch)
            .init(new OnnxEdge.Builder(flat_no_batch.literalType, flat_no_batch.shape))
            .as(OnnxEdge);
        ...
        if (batch.length > 0) {
            ...
            const bMulMN = g
                ...
                .init(new TensorNode.Builder(DataType.INT64, iIdx.shape, "intermediate"))
                .as(TensorNode);
            g.addEdge(bMulMN_node, bMulMN)
                .init(new OnnxEdge.Builder(bMulMN.literalType, bMulMN.shape))
                .as(OnnxEdge);
            ...
            flat = g
                ...
                .init(new TensorNode.Builder(DataType.INT64, iIdx.shape, "intermediate"))
                .as(TensorNode);
            g.addEdge(flat_with_b_node, flat)
                .init(new OnnxEdge.Builder(flat.literalType, flat.shape))
                .as(OnnxEdge);
        }
        ...
        const a_row = g
            ...
            .as(TensorNode);
        g.addEdge(a_row_node, a_row)
            .init(new OnnxEdge.Builder(a_row.literalType, a_row.shape))
            .as(OnnxEdge);
        ...
        const a_scalar = g
            ...
            .init(new TensorNode.Builder(elemTy, [1], "intermediate"))
            .as(TensorNode);
        g.addEdge(a_pick_node, a_scalar)
            .init(new OnnxEdge.Builder(a_scalar.literalType, a_scalar.shape))
            .as(OnnxEdge);
        ...
        const b_col = g
            ...
            .init(new TensorNode.Builder(elemTy, [1, K], "intermediate"))
            .as(TensorNode);
        g.addEdge(b_col_node, b_col)
            .init(new OnnxEdge.Builder(b_col.literalType, b_col.shape))
            .as(OnnxEdge);
        ...
        const b_scalar = g
            ...
            .init(new TensorNode.Builder(elemTy, [1], "intermediate"))
            .as(TensorNode);
        g.addEdge(b_pick_node, b_scalar)
            .init(new OnnxEdge.Builder(b_scalar.literalType, b_scalar.shape))
            .as(OnnxEdge);
        ...
        const prodT = g
            ...
            .init(new TensorNode.Builder(elemTy, [], "intermediate"))
            .as(TensorNode);
        g.addEdge(mul_node, prodT)
            .init(new OnnxEdge.Builder(prodT.literalType, prodT.shape))
            .as(OnnxEdge);
        ...
        const prev = g
            ...
            .as(TensorNode);
        g.addEdge(prev_node, prev)
            .init(new OnnxEdge.Builder(prev.literalType, prev.shape))
            .as(OnnxEdge);
