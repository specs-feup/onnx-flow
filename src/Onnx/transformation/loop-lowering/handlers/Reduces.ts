...
export default function handleReduceElem(
    ...
    const elemTy =
        accScalar.literalType !== DataType.UNDEFINED
            ? accScalar.literalType
            : xScalar.literalType !== DataType.UNDEFINED
              ? xScalar.literalType
              : DataType.FLOAT;
    ...
    const unary = (type: string, a: TensorNode.Class, name: string): TensorNode.Class => {
        const n = g
            ...
            .as(OperationNode);
        const out = g
            .addNode(uniq(g, `${name}_out_${op.id}`))
    ...
    switch (op.type) {
        ...
        case "ReduceLogSum": {
            ...
            const eq0Node = g
                .addNode(uniq(g, `ls_eq0_${op.id}`))
                .init(new OperationNode.Builder("Equal", [accScalar, zeroF], {}))
                .as(OperationNode);
            const eq0 = g
                .addNode(uniq(g, `ls_eq0_out_${op.id}`))
                .init(new TensorNode.Builder(DataType.BOOL, [], "intermediate"))
                .as(TensorNode);
            g.addEdge(eq0Node, eq0)
                .init(new OnnxEdge.Builder(eq0.literalType, eq0.shape))
                .as(OnnxEdge);

            // eq1x: x == 1
            const eq1Node = g
                .addNode(uniq(g, `ls_eq1x_${op.id}`))
                .init(new OperationNode.Builder("Equal", [xScalar, oneF], {}))
                .as(OperationNode);
            const eq1x = g
                .addNode(uniq(g, `ls_eq1x_out_${op.id}`))
                .init(new TensorNode.Builder(DataType.BOOL, [], "intermediate"))
                .as(TensorNode);
            g.addEdge(eq1Node, eq1x)
                .init(new OnnxEdge.Builder(eq1x.literalType, eq1x.shape))
                .as(OnnxEdge);

            // first_and_one = (acc==0 && x==1)
            const andNode = g
                .addNode(uniq(g, `ls_and_${op.id}`))
                .init(new OperationNode.Builder("And", [eq0, eq1x], {}))
                .as(OperationNode);
            const firstAndOne = g
                .addNode(uniq(g, `ls_and_out_${op.id}`))
                .init(new TensorNode.Builder(DataType.BOOL, [], "intermediate"))
                .as(TensorNode);
            g.addEdge(andNode, firstAndOne)
                .init(new OnnxEdge.Builder(firstAndOne.literalType, firstAndOne.shape))
                .as(OnnxEdge);
            ...
            const prevIfEq0Node = g
                .addNode(uniq(g, `ls_prev_if_eq0_${op.id}`))
                .init(new OperationNode.Builder("Where", [eq0, zeroF, expAcc], {}))
                .as(OperationNode);
            const prevIfEq0 = g
                .addNode(uniq(g, `ls_prev_if_eq0_out_${op.id}`))
                .init(new TensorNode.Builder(elemTy, [], "intermediate"))
                .as(TensorNode);
            g.addEdge(prevIfEq0Node, prevIfEq0)
                .init(new OnnxEdge.Builder(prevIfEq0.literalType, prevIfEq0.shape))
                .as(OnnxEdge);

            const prevNode = g
                .addNode(uniq(g, `ls_prev_${op.id}`))
                .init(new OperationNode.Builder("Where", [firstAndOne, tinyF, prevIfEq0], {}))
                .as(OperationNode);
            const sumPrev = g
                .addNode(uniq(g, `ls_prev_out_${op.id}`))
                .init(new TensorNode.Builder(elemTy, [], "intermediate"))
                .as(TensorNode);
            g.addEdge(prevNode, sumPrev)
                .init(new OnnxEdge.Builder(sumPrev.literalType, sumPrev.shape))
                .as(OnnxEdge);
        ...
        case "ReduceLogSumExp": {
            ...
            const eq0 = (() => {
                const n = g
                    .addNode(uniq(g, `eq0_${op.id}`))
                    .init(new OperationNode.Builder("Equal", [accScalar, zero], {}))
                    .as(OperationNode);
                const out = g
                    .addNode(uniq(g, `eq0_out_${op.id}`))
                    .init(new TensorNode.Builder(DataType.BOOL, [], "intermediate"))
                    .as(TensorNode);
                g.addEdge(n, out)
