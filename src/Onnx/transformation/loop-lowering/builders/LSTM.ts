...
export default function lowerLSTM(g: OnnxGraph.Class): void {
    ...
    for (const op of ops) {
        ...
        const [Tdim, Ndim] = xShape.map((d) => (d > 0 ? d : -1));
        const H = hiddenSize;
        ...
        const WshapeOK =
            W.shape?.length === 3 &&
            Number(W.shape[0]) === dirCount &&
            Number(W.shape[1]) === 4 * H;
        const RshapeOK =
            R.shape?.length === 3 &&
            Number(R.shape[0]) === dirCount &&
            Number(R.shape[1]) === 4 * H;
        ...
        if (!B) {
            B = makeTensorConst(g, uniq(g, `B_zero_${op.id}`), DataType.FLOAT, "constant", {
                ...
                dims: [dirCount, 8 * H],
                floatData: new Array(dirCount * 8 * H).fill(0),
            });
        }
        if (!H0) {
            H0 = makeTensorConst(g, uniq(g, `H0_zero_${op.id}`), DataType.FLOAT, "constant", {
                dataType: DataType.FLOAT,
                dims: [dirCount, Math.max(1, Ndim), H],
                floatData: new Array(dirCount * Math.max(1, Ndim) * H).fill(0),
            });
        }
        if (!C0) {
        ...
        if (dirCount === 2) {
            ...
            const catYh = g
                .addNode(uniq(g, `catYh_${op.id}`))
                .init(new OperationNode.Builder("Concat", [Yhf, Yhb], { axis: 0 }))
                .as(OperationNode);
            const Y_h = g
                .addNode(uniq(g, `Y_h_${op.id}`))
                .init(new TensorNode.Builder(DataType.FLOAT, [2, Ndim, H], "intermediate"))
                .as(TensorNode);
            g.addEdge(catYh, Y_h)
                .init(new OnnxEdge.Builder(Y_h.literalType, Y_h.shape))
                .as(OnnxEdge);

            const catYc = g
                .addNode(uniq(g, `catYc_${op.id}`))
                .init(new OperationNode.Builder("Concat", [Ycf, Ycb], { axis: 0 }))
                .as(OperationNode);
            const Y_c = g
                .addNode(uniq(g, `Y_c_${op.id}`))
                .init(new TensorNode.Builder(DataType.FLOAT, [2, Ndim, H], "intermediate"))
                .as(TensorNode);
            g.addEdge(catYc, Y_c)
                .init(new OnnxEdge.Builder(Y_c.literalType, Y_c.shape))
                .as(OnnxEdge);
...
function lowerOneDir(
    ...
    // Shapes [T,N,I]
    const xShape = toStaticShape(X.shape as Shape);
    const Tdim = xShape[0];
    const Ndim = xShape[1];
    const Idim = xShape[2];

    // Slice W/R/B for this direction: first dim = dirIndex
    const starts3 = makeTensorConst(
        g,
        uniq(g, `W_s_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([dirIndex, 0, 0]),
    );
    const endsW = makeTensorConst(
        g,
        uniq(g, `W_e_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([dirIndex + 1, 4 * H, Idim]),
    );
    const endsR = makeTensorConst(
        g,
        uniq(g, `R_e_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([dirIndex + 1, 4 * H, H]),
    );
    const axes3 = makeTensorConst(
        g,
        uniq(g, `axes3_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([0, 1, 2]),
    );
    const steps3 = makeTensorConst(
        g,
        uniq(g, `steps3_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([1, 1, 1]),
    );
    ...
    const slW = g
        .addNode(uniq(g, `Slice_W_${op.id}_${dirIndex}`))
        .init(new OperationNode.Builder("Slice", [W, starts3, endsW, axes3, steps3]))
        .as(OperationNode);
    const W01 = g
        .addNode(uniq(g, `W01_${op.id}_${dirIndex}`))
    ...
    const slR = g
        .addNode(uniq(g, `Slice_R_${op.id}_${dirIndex}`))
        .init(new OperationNode.Builder("Slice", [R, starts3, endsR, axes3, steps3]))
        .as(OperationNode);
    const R01 = g
        .addNode(uniq(g, `R01_${op.id}_${dirIndex}`))
    ...
    const trW = g
        .addNode(uniq(g, `TrW_${op.id}_${dirIndex}`))
        .init(new OperationNode.Builder("Transpose", [W01], { perm: [1, 0] }))
        .as(OperationNode);
    const Wt = g
        .addNode(uniq(g, `Wt_${op.id}_${dirIndex}`))
    ...
    const trR = g
        .addNode(uniq(g, `TrR_${op.id}_${dirIndex}`))
        .init(new OperationNode.Builder("Transpose", [R01], { perm: [1, 0] }))
        .as(OperationNode);
    const Rt = g
        .addNode(uniq(g, `Rt_${op.id}_${dirIndex}`))
    ...
    const starts2 = makeTensorConst(
        g,
        uniq(g, `B_s_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([dirIndex, 0]),
    );
    const axes2 = makeTensorConst(
        g,
        uniq(g, `axes2_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([0, 1]),
    );
    const steps2 = makeTensorConst(
        g,
        uniq(g, `steps2_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([1, 1]),
    );
    const endsB0 = makeTensorConst(
        g,
        uniq(g, `B_e0_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([dirIndex + 1, 4 * H]),
    );
    const endsB1 = makeTensorConst(
        g,
        uniq(g, `B_e1_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([dirIndex + 1, 8 * H]),
    );

    const slB0 = g
        .addNode(uniq(g, `Slice_B0_${op.id}_${dirIndex}`))
        .init(new OperationNode.Builder("Slice", [B, starts2, endsB0, axes2, steps2]))
        .as(OperationNode);
    const B0 = g
        .addNode(uniq(g, `B0_${op.id}_${dirIndex}`))
    ...
    const slB1 = g
        .addNode(uniq(g, `Slice_B1_${op.id}_${dirIndex}`))
        .init(new OperationNode.Builder("Slice", [B, starts2, endsB1, axes2, steps2]))
        .as(OperationNode);
    const B1 = g
        .addNode(uniq(g, `B1_${op.id}_${dirIndex}`))
        .init(new TensorNode.Builder(DataType.FLOAT, [1, 4 * H], "intermediate"))
        .as(TensorNode);
    g.addEdge(slB1, B1).init(new OnnxEdge.Builder(B1.literalType, B1.shape)).as(OnnxEdge);

    const ax0 = makeTensorConst(
        g,
        uniq(g, `axis0_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([0]),
    );
    const sqB0 = g
        .addNode(uniq(g, `sqB0_${op.id}_${dirIndex}`))
        .init(new OperationNode.Builder("Squeeze", [B0, ax0]))
        .as(OperationNode);
    const B0sq = g
        .addNode(uniq(g, `B0sq_${op.id}_${dirIndex}`))
    ...
    const cond = makeTensorConst(
        g,
        uniq(g, `cond_${op.id}_${direction}`),
        DataType.BOOL,
        "constant",
        bool(true),
    );
    ...
    const splitSizes = makeTensorConst(
        g,
        uniq(g, `split_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([H, H, H, H]),
    );
    ...
    const condIn = body
        ...
        .as(TensorNode);
    const Hin = body
        .addNode(uniq(body, "Hin"))
        .init(new TensorNode.Builder(DataType.FLOAT, [Ndim, H], "input"))
        .as(TensorNode);
    const Cin = body
        .addNode(uniq(body, "Cin"))
        .init(new TensorNode.Builder(DataType.FLOAT, [Ndim, H], "input"))
        .as(TensorNode);

    const Xin = body
        .addNode(uniq(body, "Xin"))
        .init(new TensorNode.Builder(DataType.FLOAT, [Tdim, Ndim, Idim], "input"))
        .as(TensorNode);
    const Wt_in = body
        .addNode(uniq(body, "Wt_in"))
        .init(new TensorNode.Builder(DataType.FLOAT, [Idim, 4 * H], "input"))
        .as(TensorNode);
    const Rt_in = body
        .addNode(uniq(body, "Rt_in"))
        .init(new TensorNode.Builder(DataType.FLOAT, [H, 4 * H], "input"))
        .as(TensorNode);
    const Bsum_in = body
        .addNode(uniq(body, "Bsum_in"))
        .init(new TensorNode.Builder(DataType.FLOAT, [4 * H], "input"))
        .as(TensorNode);
    const split_in = body
        .addNode(uniq(body, "split_sizes"))
        .init(new TensorNode.Builder(DataType.INT64, [4], "input"))
        .as(TensorNode);
    ...
    if (seqLens) {
        ...
        const cmp = body
            .addNode(uniq(body, `cmp_len_${direction}`))
            .init(new OperationNode.Builder(cmpOp, [iterU, seqLens]))
            .as(OperationNode);
        stepMask = body
            .addNode(uniq(body, `mask_${direction}`))
            .init(new TensorNode.Builder(DataType.BOOL, [1, Ndim], "intermediate"))
            .as(TensorNode);
        body.addEdge(cmp, stepMask)
            .init(new OnnxEdge.Builder(stepMask.literalType, stepMask.shape))
            .as(OnnxEdge);
    }
    ...
    if (direction === "reverse") {
        ...
        const t1 = body
            .addNode(uniq(body, "Tminus1"))
            .init(new OperationNode.Builder("Sub", [Tconst, one]))
            .as(OperationNode);
        const t1Out = body
            .addNode(uniq(body, "Tminus1_out"))
            .init(new TensorNode.Builder(DataType.INT64, [], "intermediate"))
            .as(TensorNode);
        body.addEdge(t1, t1Out)
            .init(new OnnxEdge.Builder(t1Out.literalType, t1Out.shape))
            .as(OnnxEdge);

        const sub = body
            .addNode(uniq(body, "rev_idx"))
            .init(new OperationNode.Builder("Sub", [t1Out, iter]))
            .as(OperationNode);
        const subOut = body
            .addNode(uniq(body, "rev_idx_out"))
            .init(new TensorNode.Builder(DataType.INT64, [], "intermediate"))
            .as(TensorNode);
        body.addEdge(sub, subOut)
    ...
    const mm1 = body
        .addNode(uniq(body, "mm_xw"))
        .init(new OperationNode.Builder("MatMul", [Xt, Wt_in]))
        .as(OperationNode);
    const XW = body
        .addNode(uniq(body, "XW"))
        .init(new TensorNode.Builder(DataType.FLOAT, [Ndim, 4 * H], "intermediate"))
        .as(TensorNode);
    body.addEdge(mm1, XW).init(new OnnxEdge.Builder(XW.literalType, XW.shape)).as(OnnxEdge);

    const mm2 = body
        .addNode(uniq(body, "mm_hr"))
        .init(new OperationNode.Builder("MatMul", [Hin, Rt_in]))
        .as(OperationNode);
    const HR = body
        .addNode(uniq(body, "HR"))
        .init(new TensorNode.Builder(DataType.FLOAT, [Ndim, 4 * H], "intermediate"))
        .as(TensorNode);
    body.addEdge(mm2, HR).init(new OnnxEdge.Builder(HR.literalType, HR.shape)).as(OnnxEdge);

    const add0 = body
        .addNode(uniq(body, "add0"))
        .init(new OperationNode.Builder("Add", [XW, HR]))
        .as(OperationNode);
    const pre0 = body
        .addNode(uniq(body, "pre0"))
        .init(new TensorNode.Builder(DataType.FLOAT, [Ndim, 4 * H], "intermediate"))
        .as(TensorNode);
    body.addEdge(add0, pre0).init(new OnnxEdge.Builder(pre0.literalType, pre0.shape)).as(OnnxEdge);

    const add1 = body
        .addNode(uniq(body, "add1"))
        .init(new OperationNode.Builder("Add", [pre0, Bsum_in]))
        .as(OperationNode);
    const pre = body
        .addNode(uniq(body, "pre"))
        .init(new TensorNode.Builder(DataType.FLOAT, [Ndim, 4 * H], "intermediate"))
        .as(TensorNode);
    ...
    if (seqLens) {
        ...
        const Hsel = add(
            body,
            mul(body, Hexp, maskF, "mH", [1, Ndim, H]),
            mul(body, HinU, invM, "iH", [1, Ndim, H]),
            "Hsel",
            [1, Ndim, H],
        );
        const Csel = add(
            body,
            mul(body, Cexp, maskF, "mC", [1, Ndim, H]),
            mul(body, CinU, invM, "iC", [1, Ndim, H]),
            "Csel",
            [1, Ndim, H],
        );
    ...
    // --- Outer Loop node
    const loop = g
        .addNode(uniq(g, `Loop_${op.id}_${direction}`))
        .init(
            new OperationNode.Builder(
                ...
                [
                    ...
                    makeTensorConst(
                        g,
                        uniq(g, `cond_${op.id}_${direction}_outer`),
                        DataType.BOOL,
                        "constant",
                        bool(true),
                    ),
    ...
    const H_end = g
        .addNode(uniq(g, `H_end_${op.id}_${direction}`))
        .init(new TensorNode.Builder(DataType.FLOAT, [Ndim, H], "intermediate"))
        .as(TensorNode);
    const C_end = g
        .addNode(uniq(g, `C_end_${op.id}_${direction}`))
        .init(new TensorNode.Builder(DataType.FLOAT, [Ndim, H], "intermediate"))
        .as(TensorNode);
    const X_last = g
        .addNode(uniq(g, `X_last_${op.id}_${direction}`))
        .init(new TensorNode.Builder(DataType.FLOAT, [Tdim, Ndim, Idim], "intermediate"))
        .as(TensorNode);
    const Y_scan = g
        .addNode(uniq(g, `Y_scan_${op.id}_${direction}`))
        .init(new TensorNode.Builder(DataType.FLOAT, [Tdim, Ndim, H], "intermediate"))
        .as(TensorNode);
    ...
    g.addEdge(loop, X_last)
        .init(new OnnxEdge.Builder(X_last.literalType, X_last.shape))
        .as(OnnxEdge);
    g.addEdge(loop, Y_scan)
        .init(new OnnxEdge.Builder(Y_scan.literalType, Y_scan.shape))
        .as(OnnxEdge);

    // Final public outputs
    const axesSeq = makeTensorConst(
        g,
        uniq(g, `axes_seq_pub_${op.id}_${direction}`),
        DataType.INT64,
        "constant",
        int64Vec([1]),
    );
    const axesYHC = makeTensorConst(
        g,
        uniq(g, `axes_yhc_pub_${op.id}_${direction}`),
        DataType.INT64,
        "constant",
        int64Vec([0]),
    );
    ...
    const Y_unsq = g
        .addNode(uniq(g, `unsqY_${op.id}_${direction}`))
        .init(new OperationNode.Builder("Unsqueeze", [Y_scan, axesSeq]))
        .as(OperationNode);
    const Y = g
        .addNode(uniq(g, `Y_${op.id}_${direction}`))
        .init(new TensorNode.Builder(DataType.FLOAT, [Tdim, 1, Ndim, H], "intermediate"))
        .as(TensorNode);
    g.addEdge(Y_unsq, Y).init(new OnnxEdge.Builder(Y.literalType, Y.shape)).as(OnnxEdge);

    const Yh_unsq = g
        .addNode(uniq(g, `unsqYh_${op.id}_${direction}`))
        .init(new OperationNode.Builder("Unsqueeze", [H_end, axesYHC]))
        .as(OperationNode);
    const Y_h = g
        .addNode(uniq(g, `Y_h_${op.id}_${direction}`))
        .init(new TensorNode.Builder(DataType.FLOAT, [1, Ndim, H], "intermediate"))
        .as(TensorNode);
    g.addEdge(Yh_unsq, Y_h).init(new OnnxEdge.Builder(Y_h.literalType, Y_h.shape)).as(OnnxEdge);

    const Yc_unsq = g
        .addNode(uniq(g, `unsqYc_${op.id}_${direction}`))
        .init(new OperationNode.Builder("Unsqueeze", [C_end, axesYHC]))
        .as(OperationNode);
    const Y_c = g
        .addNode(uniq(g, `Y_c_${op.id}_${direction}`))
        .init(new TensorNode.Builder(DataType.FLOAT, [1, Ndim, H], "intermediate"))
        .as(TensorNode);
...
function sliceDirNC(
    ...
    const starts = makeTensorConst(
        g,
        uniq(g, `${tag}_s_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([dirIndex, 0, 0]),
    );
    const ends = makeTensorConst(
        g,
        uniq(g, `${tag}_e_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([dirIndex + 1, N, H]),
    );
    const axes = makeTensorConst(
        g,
        uniq(g, `${tag}_axes_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([0, 1, 2]),
    );
    const steps = makeTensorConst(
        g,
        uniq(g, `${tag}_steps_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([1, 1, 1]),
    );
    const sl = g
        .addNode(uniq(g, `Slice_${tag}_${op.id}_${dirIndex}`))
        .init(new OperationNode.Builder("Slice", [T, starts, ends, axes, steps]))
        .as(OperationNode);
    const out3 = g
        .addNode(uniq(g, `${tag}_3_${op.id}_${dirIndex}`))
    ...
    const ax0 = makeTensorConst(
        g,
        uniq(g, `${tag}_ax0_${op.id}_${dirIndex}`),
        DataType.INT64,
        "constant",
        int64Vec([0]),
    );
    const sq = g
        .addNode(uniq(g, `Squeeze_${tag}_${op.id}_${dirIndex}`))
        .init(new OperationNode.Builder("Squeeze", [out3, ax0]))
        .as(OperationNode);
    const out2 = g
        .addNode(uniq(g, `${tag}_2_${op.id}_${dirIndex}`))
...
function indexFromXTime(
    ...
    if (shp[0] > 0) {
        return makeTensorConst(
            g,
            uniq(g, `trip_${X.id}_${dir}`),
            DataType.INT64,
            "constant",
            scalarInt64(shp[0]),
        );
    ...
    const shapeN = g
        .addNode(uniq(g, `shape_${X.id}_${dir}`))
        .init(new OperationNode.Builder("Shape", [X]))
        .as(OperationNode);
    const shapeOut = g
        .addNode(uniq(g, `shape_out_${X.id}_${dir}`))
        .init(new TensorNode.Builder(DataType.INT64, [3], "intermediate"))
        .as(TensorNode);
    g.addEdge(shapeN, shapeOut)
        .init(new OnnxEdge.Builder(shapeOut.literalType, shapeOut.shape))
        .as(OnnxEdge);

    const idx0 = makeTensorConst(
        g,
        uniq(g, `idx0_${X.id}_${dir}`),
        DataType.INT64,
        "constant",
        scalarInt64(0),
    );
    const g0 = g
        .addNode(uniq(g, `g0_${X.id}_${dir}`))
        .init(new OperationNode.Builder("Gather", [shapeOut, idx0], { axis: 0 }))
        .as(OperationNode);
    const out = g
        .addNode(uniq(g, `g0_out_${X.id}_${dir}`))
        .init(new TensorNode.Builder(DataType.INT64, [], "intermediate"))
        .as(TensorNode);
...
function applyAct(
    ...
    const n = body
        .addNode(uniq(body, `${tag}_${opName}`))
        .init(new OperationNode.Builder(opName, [x]))
        .as(OperationNode);
    const out = body
        .addNode(uniq(body, `${tag}_out`))
        .init(new TensorNode.Builder(DataType.FLOAT, x.shape, "intermediate"))
        .as(TensorNode);
...
function mul(
    ...
    const n = body
        .addNode(uniq(body, `mul_${tag}`))
        .init(new OperationNode.Builder("Mul", [a, b]))
        .as(OperationNode);
    const out = body
        .addNode(uniq(body, `mul_${tag}_out`))
        .init(new TensorNode.Builder(DataType.FLOAT, shape, "intermediate"))
        .as(TensorNode);
    body.addEdge(n, out).init(new OnnxEdge.Builder(out.literalType, out.shape)).as(OnnxEdge);
    return out;
}
function add(
    ...
    const n = body
        .addNode(uniq(body, `add_${tag}`))
        .init(new OperationNode.Builder("Add", [a, b]))
        .as(OperationNode);
    const out = body
        .addNode(uniq(body, `add_${tag}_out`))
        .init(new TensorNode.Builder(DataType.FLOAT, shape, "intermediate"))
        .as(TensorNode);
    body.addEdge(n, out).init(new OnnxEdge.Builder(out.literalType, out.shape)).as(OnnxEdge);
    return out;
...
function sub(
    ...
    const n = body
        .addNode(uniq(body, `sub_${tag}`))
        .init(new OperationNode.Builder("Sub", [a, b]))
        .as(OperationNode);
    const out = body
        .addNode(uniq(body, `sub_${tag}_out`))
        .init(new TensorNode.Builder(DataType.FLOAT, shape, "intermediate"))
        .as(TensorNode);
    body.addEdge(n, out).init(new OnnxEdge.Builder(out.literalType, out.shape)).as(OnnxEdge);
    return out;
...
function unsqueeze(
    ...
    const u = body
        .addNode(uniq(body, tag))
        .init(new OperationNode.Builder("Unsqueeze", [t, axes]))
        .as(OperationNode);
    const out = body
        .addNode(uniq(body, `${tag}_out`))
        .init(new TensorNode.Builder(t.literalType, [1, ...(t.shape ?? [])], "intermediate"))
        .as(TensorNode);
...
function expandTo(
    ...
    const shp = makeTensorConst(
        body,
        uniq(body, `shape_${tag}`),
        DataType.INT64,
        "constant",
        int64Vec(target),
    );
    const e = body
        .addNode(uniq(body, `Expand_${tag}`))
        .init(new OperationNode.Builder("Expand", [t, shp]))
        .as(OperationNode);
    const out = body
        .addNode(uniq(body, `Expand_${tag}_out`))
        .init(new TensorNode.Builder(t.literalType, target, "intermediate"))
        .as(TensorNode);
    body.addEdge(e, out).init(new OnnxEdge.Builder(out.literalType, out.shape)).as(OnnxEdge);
    return out;
}

function castBoolToFloat(
    body: OnnxGraph.Class,
    t: TensorNode.Class,
    tag: string,
): TensorNode.Class {
    const c = body
        .addNode(uniq(body, `cast_${tag}`))
        .init(new OperationNode.Builder("Cast", [t], { to: DataType.FLOAT }))
        .as(OperationNode);
    const out = body
        .addNode(uniq(body, `cast_${tag}_out`))
        .init(new TensorNode.Builder(DataType.FLOAT, t.shape, "intermediate"))
        .as(TensorNode);
...
function squeezeTo2(
    ...
    const axes = makeTensorConst(
        body,
        uniq(body, `sq_axes_${tag}`),
        DataType.INT64,
        "constant",
        int64Vec([0]),
    );
    const s = body
        .addNode(uniq(body, `Squeeze_${tag}`))
        .init(new OperationNode.Builder("Squeeze", [t, axes]))
        .as(OperationNode);
    const out = body
        .addNode(uniq(body, `Squeeze_${tag}_out`))
        .init(new TensorNode.Builder(DataType.FLOAT, shape2, "intermediate"))
        .as(TensorNode);
...
function identityOut(
    ...
    const n = body
        .addNode(uniq(body, `id_${name}`))
        .init(new OperationNode.Builder("Identity", [t]))
        .as(OperationNode);
    const out = body
        .addNode(name)
        .init(new TensorNode.Builder(ty, shape, "output"))
        .as(TensorNode);
    body.addEdge(n, out).init(new OnnxEdge.Builder(out.literalType, out.shape)).as(OnnxEdge);
    return out;
}
...
function rewireAndRemove(
    g: OnnxGraph.Class,
    op: OperationNode.Class,
    outs: [TensorNode.Class, TensorNode.Class, TensorNode.Class],
) {
