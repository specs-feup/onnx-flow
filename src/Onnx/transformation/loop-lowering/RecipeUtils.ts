...
import { toStaticShape, scalarInt64, int64Vec } from "../../Utils.js";
...
export function resolveRecipeInput(
    ...
    if (valueMap.has(shapeKey)) {
        ...
        const outRadix = outDims.map((d, i) => {
            if (typeof d === "number") {
                if (d > 0) return d;

                // If it's -1 (dynamic), extract the true dim from targetShapeNode
                if (targetShapeNode) {
                    const iConst = builder.createConstant(
                        `dim_idx_out_${tInner.id}_${i}`,
                        int64Vec([i]),
                    );
                    const [dimRaw] = builder.createOp("Gather", [targetShapeNode, iConst], {
                        axis: 0,
                    });
                    const sqDim = squeezeIfLen1(
                        builder,
                        dimRaw,
                        axes,
                        `sq_dim_out_${tInner.id}_${i}`,
                    );

                    // Safety check: ensure the dim is at least 1 (Max(dim, 1))
                    const oneConst = builder.createConstant(
                        `one_max_${tInner.id}_${i}`,
                        scalarInt64(1),
                    );
                    return builder.createOp("Max", [sqDim, oneConst])[0];
                }
                return 1; // Fallback
            }
            // For dynamic radix ValueNodes (if any were passed directly): Max(d, 1)
        ...
        const strides: (number | ValueNode)[] = new Array(rI).fill(1);
