...
function isSimpleMatMul(op: OperationNode.Class): boolean {
    ...
    if (aShape.some((d) => typeof d === "string") || bShape.some((d) => typeof d === "string")) {
        return false;
    }
...
function splitByMatMul(seg: OperationNode.Class[]): OperationNode.Class[][] {
    ...
    for (const op of seg) {
        if (op.type === "MatMul") {
            if (curHasMatMul) {
                // Closing current group before starting a new MatMul group
                flush();
            }
            cur.push(op);
            curHasMatMul = true;
...
function canBeEpilogue(op: OperationNode.Class, reducedOutShape: (number | string)[]): boolean {
    ...
    if (isElementwiseBinary(op)) {
        ...
        const outT = op.getOutgoers.targets
            ?.filter((n) => n.is(TensorNode))
            .first()
            ?.as(TensorNode);
...
export default class TransformChain implements Graph.Transformation<
    OnnxGraph.Class,
    OnnxGraph.Class
> {
    constructor(
        ...
        function collectChain(
            ...
            op.getInputs()?.forEach((inp) => {
                if (inp.is(TensorNode)) {
                    ...
                    if (["constant", "input", "initializer", "index", "index_aux"].includes(t.type))
                        return;
