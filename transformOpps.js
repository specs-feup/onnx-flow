...
function transformSimpleLoopOperations(node, cy, edgeOrder) {
    ...
    cy.add([
        ...
        {group: 'nodes', data: {id: formatId(node.data('opType'), nodeId), label: node.data('opType'), opType: node.data('opType')}, classes: 'operation'},
...
function transformMatMul(node, cy, edgeOrder) {
    ...
    cy.add([
        {group: 'nodes', data: {id: formatId('LoopIterations', nodeId), label: '# of loop iterations', value: numberOfIterations}, classes: 'constant'},
        {group: 'nodes', data: {id: formatId('MatMul', nodeId), label: 'MatMul', opType: 'MatMul'}, classes: 'operation'},
...
export function transformOpps(cy) {
    ...
    cy.nodes('.operation').forEach(node => {
        if (node.data('opType') === 'MatMul') {
            ...
            // é necessário realizar numpy style broadcasting?
            transformSimpleLoopOperations(node, cy, edgeOrder)
        }
    })
}

function optimizeSimpleLoopOperations(node, cy) {
    ...
    const incomingEdges = node.incomers('edge');
    const outgoingEdges = node.outgoers('edge');
    const dimensions = incomingEdges[1].data('dims');

    const nodeId = node.data('id');

    if (dimensions[0].dimValue === '1' && dimensions[1].dimValue === '1') {
        cy.remove('#' + incomingEdges[0].data('source'))
        cy.add([
            {group: 'nodes', data: {id: formatId(opType, nodeId), label: opLabel, opType: opType}, classes: 'operation'},
            {group: 'edges', data: {source: incomingEdges[1].data('source'), target: formatId(opType, nodeId), label: incomingEdges[1].data('label'), dims: incomingEdges[1].data('dims'), elemType: incomingEdges[1].data('elemType'), opType: incomingEdges[1].data('opType'), order: incomingEdges[1].data('order')}, classes: incomingEdges[1].classes()},
            {group: 'edges', data: {source: incomingEdges[2].data('source'), target: formatId(opType, nodeId), label: incomingEdges[2].data('label'), dims: incomingEdges[2].data('dims'), elemType: incomingEdges[2].data('elemType'), opType: incomingEdges[2].data('opType'), order: incomingEdges[2].data('order')}, classes: incomingEdges[2].classes()}
        ]);
        outgoingEdges.forEach(edge => {
            cy.add({group: 'edges', data: {source: formatId(opType, nodeId), target: edge.data('target'), label: edge.data('label'), dims: edge.data('dims'), elemType: edge.data('elemType'), opType: opType, order: edge.data('order')}, classes: 'operation variable outer'});
        })
        cy.remove(node)
    }
}

function optimizeMatMul(node, cy) {

    const incomingEdges = node.incomers('edge')

    cy.remove('#' + incomingEdges[0].data('source'))
    const outgoingEdges = node.outgoers('edge')
    const dimensions0 = incomingEdges[1].data('dims')
    const dimensions1 = incomingEdges[2].data('dims')
    const type = incomingEdges[1].data('elemType')
    let numberOfIterations = dimensions0[0].dimValue * dimensions1[0].dimValue * dimensions1[1].dimValue
    ...
    if (pattern === '111') {
        cy.add([
            ...
            {group: 'edges', data: {source: incomingEdges[1].data('source'), target: formatId('Multiplication', nodeId), dims: incomingEdges[1].data('dims'), elemType: incomingEdges[1].data('elemType'), opType: incomingEdges[1].data('opType'), order: incomingEdges[1].data('order')}, classes: incomingEdges[1].classes()},
            {group: 'edges', data: {source: incomingEdges[2].data('source'), target: formatId('Multiplication', nodeId), dims: incomingEdges[2].data('dims'), elemType: incomingEdges[2].data('elemType'), opType: incomingEdges[2].data('opType'), order: incomingEdges[2].data('order')}, classes: incomingEdges[2].classes()},
        ])
        outgoingEdges.forEach(edge => {
            cy.add({group: 'edges', data: {source: formatId('Multiplication', nodeId), target: edge.data('target'), dims: edge.data('dims'), elemType: edge.data('elemType'), order: edge.data('order'), opType: 'Multiplication'}, classes: 'operation variable outer'})
        })
        cy.remove(node)
        return
    }
    ...
    cy.add([
        ...
        {group: 'edges', data: {source: formatId('LoopIterations', nodeId), target: formatId('MatMul', nodeId), order: incomingEdges[0].data('order'), value: numberOfIterations}, classes: 'constant'},
        {group: 'edges', data: {source: incomingEdges[1].data('source'), target: formatId('MatMul', nodeId), dims: incomingEdges[1].data('dims'), elemType: incomingEdges[1].data('elemType'), opType: incomingEdges[1].data('opType'), order: incomingEdges[1].data('order')}, classes: incomingEdges[1].classes()},
        {group: 'edges', data: {source: incomingEdges[2].data('source'), target: formatId('MatMul', nodeId), dims: incomingEdges[1].data('dims'), elemType: incomingEdges[1].data('elemType'), opType: incomingEdges[2].data('opType'), order: incomingEdges[2].data('order')}, classes: incomingEdges[2].classes()},
    ...
    outgoingEdges.forEach(edge => {
        cy.add({group: 'edges', data: {source: formatId('MatMul', nodeId) , target: edge.data('target'), dims: edge.data('dims'), elemType: edge.data('elemType'), order: edge.data('order')}, classes: 'compound variable'})
    })
    cy.remove(node)
}
...
export function optimizeForDimensions(cy) {
    ...
    operationNodesWithoutParents.forEach(node => {
        if (node.data('opType') === 'MatMul') {
            ...
            optimizeSimpleLoopOperations(node,cy)
        }
    })
}
